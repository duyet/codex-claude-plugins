...
Clone this repository and copy the skill directory:

```bash
git clone https://github.com/duyet/clickhouse-monitoring.git
cp -r clickhouse-monitoring ~/.agents/skills/
```
...
cc --plugin-dir /path/to/clickhouse-monitoring

# Or copy to your skills directory
cp -r . ~/.agents/skills/clickhouse-monitoring
```
...
The API list and dashboard pages are automatically generated from the main codebase:

```bash
bun run scripts/sync-docs.ts
```

This scans the main `clickhouse-monitor` repository's `lib/api/charts/` and `lib/query-config/` to extract all available endpoints.
...
├── SKILL.md                 # Main skill file (YAML frontmatter + content) at root
├── README.md               # This file - installation and overview
├── LICENSE                 # MIT License
├── scripts/                # Utility scripts
│   └── sync-docs.ts        # Sync docs from main repository
...
- [ClickHouse Monitor (Main Repository)](https://github.com/duyet/clickhouse-monitor) - The actual monitoring dashboard application
