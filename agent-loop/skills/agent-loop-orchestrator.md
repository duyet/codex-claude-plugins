---
description: >-
  Core orchestrator skill for the agent-loop plugin. Runs a continuous
  maintenance loop that wakes every N minutes, scans repo queues, dispatches
  work to parallel agent threads, tracks progress, and reports results.
  Designed for overnight and day-long autonomous repo upkeep.
---
...
```bash
# Spawn parallel worker threads for each autonomous item
task(
  category="deep",
  load_skills=["agent-loop-triage", "agent-loop-autoreview", "agent-loop-browser"],
  run_in_background=true,
  prompt="=== WORKER AGENT ===\n...\nTASK: Implement fix for GH-{issue_number}: {title}\n..."
)
```
